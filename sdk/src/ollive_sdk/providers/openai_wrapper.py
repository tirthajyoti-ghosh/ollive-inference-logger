...
class OpenAIWrapper(AbstractProviderWrapper):
    ...
    def wrap(self, module: Any) -> None:
        ...
        # Async client — openai.AsyncOpenAI().chat.completions.create
        # We patch the class method on the module-level resources if present.
        async_chat = getattr(module, "AsyncOpenAI", None)
        if async_chat is not None:
            # Store for potential unwrap — but async patching is done via
            # the class prototype, handled at instance level by the user.
            pass
